...
{"milestone": "## Milestone 6 — Verifiable Trace Provenance and Recomputable Training Datasets\n\n- [ ] Add a `dataset-manifest.schema.json` (trace-pipeline-owned) that records the exact evomerge version, canonical schema versions, config hash, and input trace digest for every exported training dataset.\n- [ ] Compute a Merkle root over per-record hashes so any dataset subset can be proven to derive from a given set of input AEP/compliance records.\n- [ ] Extend `evomerge/export/` so all training record writers (SFT, DPO, PPO) emit and persist the manifest alongside the export.\n- [ ] Add `evomerge provenance verify` command that recomputes hashes from raw traces and confirms the manifest root matches, failing loudly on mismatch.\n- [ ] Store manifests in a local append-only ledger (or sidecar for object storage), enabling offline audit without re-running the pipeline.\n- [ ] Add CI check that the schema-check/reproducer tests include a provenance round-trip: export -> mutate trace -> verify detects mutation.\n- [ ] Support selective re-materialization: given a trace set and a changed schema revision, recompute only affected training records using a manifest dependency graph rather than full re-export.\n- [ ] Introduce deterministic ordering in the export pipeline (stable sort by trace ID and sequence) so manifest hashes are reproducible across runs and machines.\n- [ ] Add `--provenance-filter` to the CLI to slice a dataset by provenance attributes (e.g., agent version, compliance profile) without re-export.\n- [ ] Document the provenance format in `schemas/README` and publish the manifest schema alongside the evomerge package."}
